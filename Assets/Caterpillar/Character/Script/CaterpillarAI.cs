...
public class CaterpillarAI : VersionedMonoBehaviour, IAstarAI
{
    /// <summary>
    ...
    public bool reachedDestination
    {
        get
        {
            ...
            if (remainingDistance + dir.magnitude >= endReachedDistance) return false;
    ...
    {
        bool reachedEndOfPreviousPath = interpolator.valid && interpolator.remainingDistance < endReachedDistance;
    ...
    protected virtual Vector3 CalculateNextPosition(out Vector3 direction, float deltaTime)
    {
        ...
        float slowdown = interpolator.remainingDistance < slowdownDistance ? Mathf.Sqrt(interpolator.remainingDistance / slowdownDistance) : 1.0f;

        interpolator.distance += deltaTime * slowdown * speed;
        ...
        {
            if (float.IsNaN(interpolator.position.x) || float.IsNaN(interpolator.position.y) || float.IsNaN(interpolator.position.z))
            {
                Debug.Break();
    ...
    protected void OnDrawGizmos()
    {
        ...
        float alpha = 1.0f;
        Draw.Gizmos.Line(position, (this as IAstarAI).steeringTarget, GizmoColor * new Color(1, 1, 1, alpha));
