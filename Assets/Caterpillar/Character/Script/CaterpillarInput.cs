...
public class CaterpillarInput : MonoBehaviour
{
    #region  Fitting
    ...
    public float LookAheadRaycast = 0.3f;
    public float AheadBlend = 0.25f;
    ...
    protected virtual void FitToGround()
    {
        ...
        Vector3 RaycastDirection = (-GetUpVector() + transform.forward).normalized;

        if (LookAheadRaycast != 0f)
            Physics.Raycast(transform.position + GetUpVector() * RaycastHeightOffset + transform.forward * LookAheadRaycast, RaycastDirection, out aheadHit, RaycastCheckRange + YOffset, GroundLayerMask, QueryTriggerInteraction.Ignore);
        ...
        if (LastRaycast.transform)
        {
            ...
            {
                Debug.Log("Casted : " + aheadHit.transform.name);
    ...
    public bool fittingEnabled = true;
    ...
    public void OnMove(Vector2 Move)
    {
        ...
        if ( Move.y >= 0.0f)
        {
            if (Move.x >= 0.0f)
            {
                RotationOffset = Mathf.Lerp(0, -90, Move.x);
            }
            else
            {
                RotationOffset = Mathf.Lerp(0, 90, -Move.x);
        ...
        else
        {
            if (Move.x >= 0.0f)
            {
                RotationOffset = Mathf.Lerp(-180,-90, Move.x);
            }
            else
            {
                RotationOffset = Mathf.Lerp(180, 90, - Move.x);
